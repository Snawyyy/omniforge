...
class Refactor:
    ...
    def generate_plan(self):
        ...
        try:
            ...
            raise Exception(f"Failed to generate and validate refactor plan: {e}")
    ...
    def request_structured_plan(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle cases where the model returns a raw string
    ...
    def validate_steps(self, steps: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, step in enumerate(steps):
            ...
            if missing_fields:
                raise ValueError(f"Step {i} is missing required fields: {missing_fields}")

            if step['type'] not in modification_types:
                raise ValueError(f"Step {i} has an invalid type '{step['type']}'. Must be one of {modification_types}.")
            ...
            if step['type'] == 'edit' and 'search' not in step and 'line_number' not in step:
                raise ValueError(f"Edit step {i} must include either 'search' or 'line_number'.")
